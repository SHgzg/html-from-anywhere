...
import { ExecutableConfig, RuntimeContext, DataResult, RenderResult } from '@report-tool/types';
...
class LifecycleTester {
  ...
  private config?: ExecutableConfig;
  private dataResults?: DataResult[];
  private renderResults?: Map<string, RenderResult>;

  /**
   * 运行完整测试
   */
  async runTest(configPath: string = './config.test.json'): Promise<TestReport> {
    console.log('========================================');
   ...
   * 测试 Bootstrap 阶段
   */
  private async testBootstrap(): Promise<void> {
    const startTime = Date.now();
    console.log('\n--- Phase 1: Bootstrap ---');
   ...
   * 测试 Config 阶段
   */
  private async testConfig(configPath: string): Promise<void> {
    const startTime = Date.now();
    console.log('\n--- Phase 2: Config ---');
   ...
   * 测试 Render 阶段
   */
  private async testRender(): Promise<void> {
    const startTime = Date.now();
    console.log('\n--- Phase 4: Render ---');
   ...
   * 测试 Action 阶段
   */
  private async testAction(event: 'data_ready' | 'report_ready'): Promise<void> {
    const startTime = Date.now();
   ...
   * 打印验证信息
   */
  private printValidation(label: string, data: any): void {
    console.log(`  ${label}:`);
...
export { LifecycleTester, TestReport, TestResult };
